...
import { User, Layers, Grid, Save, Plus, Trash2, Edit2, ShieldCheck, Mail, Key, Circle, Square, RectangleHorizontal, ArrowLeft, X, Clock } from 'lucide-react';
import { TableData, TableShape, TurnTimeConfig } from '../lib/types';
...
const SettingsView: React.FC<SettingsViewProps> = ({
    ...
    const [newTableConfig, setNewTableConfig] = useState<{ name: string, floor: string, seats: number, shape: TableShape, turnTimeConfig: TurnTimeConfig }>({
        name: '', floor: floors[0] || '', seats: 2, shape: 'square', turnTimeConfig: { small: 75, medium: 105, large: 135 }
    });

    // Edit Table State
    const [editingTableId, setEditingTableId] = useState<string | null>(null);
    const [editTableConfig, setEditTableConfig] = useState<{ name: string, floor: string, seats: number, shape: TableShape, turnTimeConfig: TurnTimeConfig } | null>(null);
    ...
    const handleSaveNewTable = () => {
        if (newTableConfig.name.trim() && newTableConfig.floor) {
            onAddTable(newTableConfig.floor, newTableConfig.name, newTableConfig.seats, newTableConfig.shape); // Passing turnTimeConfig to AddTable needs FloorManager update, will just pass it to onUpdateTable after creation or update AddTable prop. Wait, AddTable signature isn't updated yet.
            setIsAddingTable(false);
            setNewTableConfig({ name: '', floor: floors[0] || '', seats: 2, shape: 'square', turnTimeConfig: { small: 75, medium: 105, large: 135 } });
        }
    };
    ...
    const startEditingTable = (table: TableData) => {
        ...
        setEditTableConfig({
            ...
            shape: table.shape,
            turnTimeConfig: table.turnTimeConfig || { small: 75, medium: 105, large: 135 }
        });
    };
    ...
    const saveEditedTable = (id: string) => {
        if (editTableConfig) {
            onUpdateTable(id, {
                ...
                turnTimeConfig: editTableConfig.turnTimeConfig
            });
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-10">
                    ...
                    {activeTab === 'tables' && (
                        <div className="h-full flex flex-col max-w-6xl mx-auto lg:mx-0">
                            ...
                            {/* ADD TABLE FORM */}
                            <div className={`overflow-hidden transition-all duration-300 ${isAddingTable ? 'max-h-auto opacity-100 mb-4' : 'max-h-0 opacity-0'}`}>
                                <div className="bg-aura-card border border-aura-primary/30 p-4 rounded-xl flex flex-col gap-4">
                                    ...
                                    {/* Turn Time Config for New Table */}
                                    <div className="pt-3 border-t border-aura-border flex items-center gap-4">
                                        <div className="text-xs text-gray-400 flex items-center gap-1 w-32"><Clock size={14} /> Turn Time</div>
                                        <div className="flex items-center gap-4 flex-1">
                                            <div className="flex items-center gap-2">
                                                <span className="text-[10px] text-gray-500 uppercase">1-2 pax:</span>
                                                <input type="number" step="15" value={newTableConfig.turnTimeConfig.small} onChange={e => setNewTableConfig({ ...newTableConfig, turnTimeConfig: { ...newTableConfig.turnTimeConfig, small: parseInt(e.target.value) || 75 } })} className="w-14 bg-aura-black border border-aura-border rounded px-2 py-1 text-xs text-white" />
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <span className="text-[10px] text-gray-500 uppercase">3-5 pax:</span>
                                                <input type="number" step="15" value={newTableConfig.turnTimeConfig.medium} onChange={e => setNewTableConfig({ ...newTableConfig, turnTimeConfig: { ...newTableConfig.turnTimeConfig, medium: parseInt(e.target.value) || 105 } })} className="w-14 bg-aura-black border border-aura-border rounded px-2 py-1 text-xs text-white" />
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <span className="text-[10px] text-gray-500 uppercase">6+ pax:</span>
                                                <input type="number" step="15" value={newTableConfig.turnTimeConfig.large} onChange={e => setNewTableConfig({ ...newTableConfig, turnTimeConfig: { ...newTableConfig.turnTimeConfig, large: parseInt(e.target.value) || 135 } })} className="w-14 bg-aura-black border border-aura-border rounded px-2 py-1 text-xs text-white" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <tbody className="divide-y divide-aura-border/30">
                                        {filteredTables.sort((a, b) => a.floor.localeCompare(b.floor) || a.name.localeCompare(b.name)).map(table => (
                                            <React.Fragment key={table.id}>
                                                <tr className={`hover:bg-white/5 transition-colors group ${editingTableId === table.id ? 'bg-white/5' : ''}`}>
                                                    {/* EDIT MODE TOP ROW */}
                                                        ...
                                                        <>
                                                            ...
                                                            <td className="p-4 text-right">
                                                                <div className="flex items-center justify-end gap-2">
                                                                    <button onClick={() => saveEditedTable(table.id)} className="p-1 text-aura-primary hover:bg-aura-primary/10 rounded" title="Salva modifiche"><Save size={16} /></button>
                                                                    <button onClick={() => setEditingTableId(null)} className="p-1 text-gray-500 hover:text-white rounded" title="Annulla"><X size={16} /></button>
                                                                </div>
                                                            </td>
                                                        </>
                                                    ) : (
                                                ...
                                                {/* EDIT MODE BOTTOM ROW (TURN TIME) */}
                                                {editingTableId === table.id && editTableConfig && (
                                                    <tr className="bg-white/5 border-b-2 border-aura-primary/50">
                                                        <td colSpan={5} className="p-4 pt-0">
                                                            <div className="flex items-center gap-6 bg-aura-black/50 p-3 rounded-xl border border-aura-primary/20">
                                                                <div className="text-xs font-bold text-aura-primary flex items-center gap-1 uppercase tracking-wide">
                                                                    <Clock size={14} /> Turn Time
                                                                </div>
                                                                <div className="flex items-center gap-4 flex-1">
                                                                    <div className="flex items-center gap-2">
                                                                        <span className="text-[10px] text-gray-400 uppercase">1-2 pax</span>
                                                                        <input type="number" step="15" value={editTableConfig.turnTimeConfig.small} onChange={e => setEditTableConfig({ ...editTableConfig, turnTimeConfig: { ...editTableConfig.turnTimeConfig, small: parseInt(e.target.value) || 75 } })} className="w-14 bg-aura-black border border-aura-border focus:border-aura-primary rounded px-2 py-1 text-xs text-white text-center" />
                                                                    </div>
                                                                    <div className="h-4 w-px bg-aura-border"></div>
                                                                    <div className="flex items-center gap-2">
                                                                        <span className="text-[10px] text-gray-400 uppercase">3-5 pax</span>
                                                                        <input type="number" step="15" value={editTableConfig.turnTimeConfig.medium} onChange={e => setEditTableConfig({ ...editTableConfig, turnTimeConfig: { ...editTableConfig.turnTimeConfig, medium: parseInt(e.target.value) || 105 } })} className="w-14 bg-aura-black border border-aura-border focus:border-aura-primary rounded px-2 py-1 text-xs text-white text-center" />
                                                                    </div>
                                                                    <div className="h-4 w-px bg-aura-border"></div>
                                                                    <div className="flex items-center gap-2">
                                                                        <span className="text-[10px] text-gray-400 uppercase">6+ pax</span>
                                                                        <input type="number" step="15" value={editTableConfig.turnTimeConfig.large} onChange={e => setEditTableConfig({ ...editTableConfig, turnTimeConfig: { ...editTableConfig.turnTimeConfig, large: parseInt(e.target.value) || 135 } })} className="w-14 bg-aura-black border border-aura-border focus:border-aura-primary rounded px-2 py-1 text-xs text-white text-center" />
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </td>
